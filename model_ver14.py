...
def get_array(csv_position):
    ...
    for i in range(len(files)):#フォルダ内のファイル数だけ繰り返す
        ...
        if(read_mode == 0):
            ...
            np.savetxt(files[0] + '_realforce.csv',real_force[TP:,:],delimiter=',')
...
def predict_thre(model_position):
    ...
    filename_predict = os.listdir(check_position[0])
    ...
    for i in range(number_of_split):
        ...
        if (temp_recall + beta_f**2*temp_precision == 0):
            F_measure = 0
        else:
            F_measure = (1+beta_f**2)*temp_recall*temp_precision/(temp_recall + beta_f**2*temp_precision)        
    ...
    np.savetxt(filename_predict[0]+'_beta'+str(beta_f)+'_actual.csv',y_check,delimiter=',')
    np.savetxt(filename_predict[0]+'_beta'+str(beta_f)+'_predicted.csv',seq_predicted,delimiter=',')
    np.savetxt(filename_predict[0]+'_beta'+str(beta_f)+'_correct.csv',correct_count,delimiter=',')
...
def calculate_threshold(model_position):
    ...
    for h in range(loop_number+1):
        ...
        np.savetxt('actual_'+ str(h)+'.csv',y_check,delimiter=',')
        np.savetxt('predicted_'+ str(h)+'.csv',seq_predicted,delimiter=',')
        np.savetxt('correct_'+ str(h)+'.csv',correct_count,delimiter=',')
