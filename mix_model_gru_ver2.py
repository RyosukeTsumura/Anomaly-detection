...
model_read_position = './gru_model/read'
...
def get_array(csv_position):
    ...
    for i in range(len(files)):#フォルダ内のファイル数だけ繰り返す
        ...
        if(read_mode == 0):
            ...
            temp_y = pd.read_csv(csv_position[0] + '/' + files[i],usecols=['normal_state','abnormal_state1','abnormal_state2'])
...
def build_model_cl(indim, outdim):#分類学習用モデル
    ...
    optimizer = Adam(lr=0.0001, beta_1=0.9, beta_2=0.999,clipnorm = 1)
...
def predict(model_position):
    ...
    np.savetxt('actual_gru.csv',y_check,delimiter=',')
    np.savetxt('predicted_gru.csv',seq_predicted,delimiter=',')
    print('save_complete')
